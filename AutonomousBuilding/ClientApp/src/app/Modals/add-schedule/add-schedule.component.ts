...
export class AddScheduleComponent implements OnInit {
  ...
  add() {
    ...
    if ((this.selecteItem !== "") && (this.selectedItem !== "")) {
      ...
      sch.number = 0
      this.http.post<ScheduleData>('/api/schedule/test', sch, httpOptions).subscribe(() => this.activeModal.close())
    }
  }
  ...
  addit() {
    ...
    if ((this.dayselected() === true) && ((this.startime() === true) && (this.time() === true))) {
      this.http.post<ScheduleData>('/api/schedule/test', sc, httpOptions).subscribe(() => this.activeModal.close())
    }
  }
